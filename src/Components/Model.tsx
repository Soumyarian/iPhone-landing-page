...
import { useColorSectionContext } from "../Context/ColorSectionProvider";

export function Model(props: any) {
  const { nodes, materials } = useGLTF("/model.glb");
  const camera = useThree(state => state.camera);
  const scene = useThree(state => state.scene);
  const { textColor } = useColorSectionContext();

  useEffect(() => {
    materials.Body.color.set(textColor);
  }, [textColor]);
  ...
  useLayoutEffect(() => {
    ...
    let tl = gsap
      .timeline({
        scrollTrigger: {
          trigger: "#enviournment",
          start: "top+=200 top",
          end: "top top",
          endTrigger: "#colorSection",
          scrub: true,
        },
      })
      .fromTo(camera.position, { y: 2 }, { y: 0 })
      .to(scene.rotation, { y: 0.8 })
      .to(scene.rotation, { y: 3 }, "key0")
      .to(scene.rotation, { z: 1.55 }, "key0")
      .to(scene.position, { x: 1 }, "key11")
      .to(scene.rotation, { z: 0 }, "key11")
      .to(scene.rotation, { y: 12.7 }, "key")
      .to(scene.position, { x: -1.15 }, "key")
      .to(scene.position, { x: -0.55 }, "key1")
      .to(scene.rotation, { z: 0 }, "key1")
      .to(camera.position, { z: 4.5 }, "key1");
    return () => {
      tl.kill();
    };
  }, []);
